...
const ContactNoteItem = ({ note, onDelete }) => (
...
const ContactNoteList = ({ notes = [], onDelete, onAdd, newNote, onNoteChange, showInput, onShowInput, onHideInput }) => (
  <div className="ml-11 mt-2 space-y-2">
    {notes.length > 0 && (
      <div className="border-t border-gray-200 pt-2 space-y-2">
        {notes.map(note => (
          <ContactNoteItem key={note.id} note={note} onDelete={onDelete} />
        ))}
      </div>
    )}
    {showInput ? (
